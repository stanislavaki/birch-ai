...
- **Все ассеты сайта хранятся в Webflow Assets** (правило от 25.09.2026) — картинки, иконки, видео и шрифты, на которые ссылаются embed-блоки и кастом-код, загружаются в Webflow Assets (своя папка на блок, напр. `home-slides-sept-26`) и подключаются с CDN сайта `cdn.prod.website-files.com/<site_id>/<file>`. Никаких внешних хостингов: ни GitHub Pages, ни личных аккаунтов, ни S3-адресов из ответа API. Причина: коллаж главной полгода грузился с GitHub Pages чужого аккаунта (`stanislavaki.github.io`) — переименуй кто-то репозиторий или выключи Pages, главная осталась бы без картинок, а обновление картинки молча зависело от слияния в `main`. Как делать: генератор embed берёт адреса из таблицы соответствий «путь в репо → имя файла на CDN» (пример — `scripts/home-slides-webflow-assets.json`) и **падает**, если картинки в таблице нет, чтобы непривязанный файл не уехал внешней ссылкой. Новый или заменённый ассет: загрузить в Webflow → прочитать имя через MCP `data_assets_tool` `list_assets` по папке → добавить в таблицу → пересобрать → сверить байты CDN с локальным файлом. Замена картинки = новый адрес на CDN, поэтому после неё заново вставляется и разметка embed.
