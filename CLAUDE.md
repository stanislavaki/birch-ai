...
- **preview_screenshot — артефакт после скролла с fixed-навбаром** — `position: fixed` навбар с `backdrop-filter: blur` ломает `preview_screenshot` после программного `window.scrollTo()` / `scrollIntoView()`: скриншот рендерится так, будто скролла не было, либо с визуальными артефактами блюра на месте нава. Это баг превью-тула, не вёрстки. Обходной путь: перед скриншотом временно скрыть навбар — `document.querySelector('.sim-nav').style.display = 'none'`, затем вернуть обратно (`= ''`) после проверки.
