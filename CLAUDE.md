...
- **Плавающее меню на мобилке — reparenting узла, а не дубль разметки** (референс — `use-cases.html`). Когда на десктопе контрол живёт в потоке (сайдбар-аккордеон, таб-бар), а на мобилке его надо собрать в плавающее меню (FAB + bottom-sheet), **не дублировать разметку** — физически переносить тот же узел в шит и обратно через `matchMedia('(max-width: 767px)')` с `initMenu()/destroyMenu()` и guard-переменной. Работает без переподвязки, потому что клики делегированы на `document` (`e.target.closest(...)`), а scroll-spy/observers ключатся по `id` — узлу всё равно, где он в DOM. Обязательные детали:
  ...
  - **Body scroll-lock, пока шит открыт** — `document.body.style.overflow = 'hidden'` в `openSheet()`, восстановить в `closeSheet()`. В репозитории это первый scroll-lock; `position: fixed`-трюк с сохранением `scrollY` не нужен, `overflow: hidden` не сбрасывает позицию.
  ...
  - **z-index:** бар/FAB держать выше backdrop'а шита, чтобы оставаться видимым и кликабельным при открытом меню (у нас FAB `3100` > backdrop `3000`), z-index оверлеев — выше фикс-навбара Webflow (`1000`), см. правило про `.sim-nav`/панель.
  - Закрытие — по backdrop / Esc / повторному тапу бара / выбору пункта; `prefers-reduced-motion` гейтит анимации выезда. Иконку-триггер удобно поворачивать: `+`→`×` через `transform: rotate(45deg)` на `.is-open`.
