...
| Новый кейс (case study) в Webflow CMS: поля записи, ассеты, спец-блоки тела (карточка стратегии, pro-tip), публикация, кастом-код шаблона | `docs/case-studies.md` |
| Версионирование юр-документов (`/legal/*`): перенос в CMS, схема коллекции, порядок катовера, типографика тела | `docs/legal-versioning-plan.md` |
...
- **Git branch** — always create a feature branch at the start of every session before making any changes. Use the format `feature/<short-description>`. Never work directly on `main`. **Сначала свежий ствол:** ветка новой сессии создаётся от *локального* `main`, который никто не обновляет (02.09.2026: ветки живых сессий стартовали на 2, 3, 5 и 64 коммита позади `origin/main`). Поэтому первое действие сессии, пока в ветке ноль своих коммитов: `git fetch origin && git merge --ff-only origin/main` — ветка встаёт на актуальный ствол. В worktree обновляем именно ветку, а не `main` (он занят главной папкой). Если в ветке уже есть коммиты и она отстала — `git merge origin/main` в ветку, не rebase (ветка могла быть запушена). Пользователю про pull писать не нужно; «загрузи контекст» этот шаг не заменяет, он делается всегда.
- **Слияние в `main` — только через pull request** — репозиторий общий, и вся история слияний здесь идёт через PR (`Merge pull request #4…#7`). Даже если сказано «слей в main», это значит **оформить PR**, а не сделать `git push origin HEAD:main`. Порядок: `git push origin <ветка>` → `gh pr create --base main` → слияние после ревью. Прямой push в `main` — только если это сказано отдельно и явно. **PR открывается только по команде пользователя** («оформи PR», «слей в main»), никогда по собственной инициативе в конце задачи (правило от 02.09.2026: PR был открыт без запроса, пока пользователь ещё смотрел результат). Предложить PR можно и по своей инициативе, если считаешь, что пора, но только вопросом: «задача выглядит закрытой — оформить PR? совет по merge — …» — и ждать ответа. Вопрос ≠ действие. **Коммит и push — без команды** (правило от 02.09.2026): коммит после каждого закрытого шага (блок встал, мобилка ок, правило дописано) с отчётом одной строкой «закоммитил: …»; push только своей ветки, тоже сам — это резервная копия, `main` не трогает. Пользователь может сказать «не коммить пока» — тогда ждать. Три уровня по весу: коммит и push обратимы и живут в своей ветке → сами; PR трогает общий `main` → только по слову пользователя. **PR открывается один раз, в конце задачи, а не на каждую правку** (правило от 01.09.2026): доковые коммиты копятся в ветке по ходу работы, и в `main` уезжают одним PR, когда задача закрыта. Причина: PR не «доставляет» изменение — правки в Webflow уже живут на сайте, — а для diff'а из одних доков ревью-ценность близка к нулю, поэтому PR на каждый коммит это шум. Если внутри задачи вырастает отдельная неродственная фаза, у неё свой PR в конце этой фазы. Правило появилось после того, как 40 коммитов уехали в `main` fast-forward'ом без ревью: откатывать не стали, потому что переписывать общую историю хуже.
...
- **Вписывать набор картинок в рамку — `width` + `height: auto` + `max-height`, а не пара `max-*`.** Пара `max-width`/`max-height` умеет только уменьшать: элемент с собственным размером меньше рамки так и останется мелким. Чтобы разноформатные картинки (логотипы клиентов, иконки, превью) встали одним визуальным весом, задавать **реальную** ширину бокса + `height: auto` + потолок высоты + `object-fit: contain` и `object-position` для прижатия к краю. Тогда узкие упираются в высоту, широкие — в ширину. Реальный кейс: логотипы кейсов жили на одном `max-height: 2rem` — Keiki рисовался 88×35, Empros 354×35 (разница по площади в 5 раз); первая попытка починить парой `max-*` ничего не изменила, потому что SVG с собственной высотой 36px не увеличивается. **Следствие для нарезки:** как только ограничена и ширина, файл обязан быть обрезан вплотную по знаку — пустые поля внутри файла теперь уменьшают сам знак.
- **Ассеты от клиента смотреть глазами до загрузки** — отрендерить на белом и прочитать надпись по буквам, проверить пропорции и обрезку. Реальный кейс: присланный клиентом логотип Empros читался «EMRPOS» (переставлены буквы); ошибку нашли уже после сборки кейса, пришлось искать логотип в шапке сайта клиента и перезаливать. **Глифы в чужом растре не править** — просить корректный файл или брать с сайта клиента; перекраска (белый → чёрный) допустима с согласия пользователя, переделка букв — нет. Найденное сразу показывать пользователю, решение за ним.
...
- **Брифинг перед версткой** — когда пользователь присылает новый блок или страницу для верстки, сначала открой макет через Figma MCP и задай все уточняющие вопросы. Не приступай к верстке пока не получены ответы. Вопросы по категориям:
  ...
  - **Состояние и шаринг (URL)**: есть ли на странице переключаемое состояние, на которое пользователь может захотеть дать прямую ссылку — активный таб/фильтр, раскрытая секция аккордеона, конкретная карточка/итем? Если да — спросить, нужно ли пробрасывать это в URL (shareable / deep-link): фильтр-таб обычно в query-параметр, конкретный итем — в хэш. Это отдельная фича (History API + разбор deep-link на загрузке, фолбэк если цель скрыта фильтром), её планировать с брифинга, а не прикручивать потом. Реализация и грабли — `docs/webflow-mcp.md` («Шаринг состояния через URL»).
...
- **Letter-spacing — шкала по кеглю, а не по платформе/брейкпоинту** — одинаковый кегль в одинаковой роли получает один трекинг везде (десктоп, мобилка, embeds). Значения задавать ярусами («24px → X, 16px → Y, 12px → Z»), а не поэлементно: иначе один и тот же 16px-заголовок на разных платформах звучит по-разному (реальный кейс: сабменю нового меню унаследовало −0.026em из мобильного макета при десктопных +0.004em — рассинхрон заметил арт-дир). Мелкому кеглю нужен больший плюс, крупному допустим ноль/минус; жирному начертанию (600+) не давать больше ~0.02em — слово рассыпается на буквы.

- **Курсив — только тегом/классом, никогда `font-style: italic`** — наклон на сайте включается осью шрифта: `em, i, cite, .u-italic { --slnt: 10 }` поверх глобального пина `font-variation-settings: "slnt" var(--slnt, 0)`. Ключевое слово `italic` движки резолвят против оси `slnt` по-разному (Chrome ось не трогает вовсе), а на iOS CoreText вдобавок может сам подобрать италиковый инстанс — отсюда был фантомный курсив на айфоне при прямом тексте. Поэтому `font-style` глобально держим `normal`, а курсив ставим `<em>`/`<i>` или классом `u-italic`; в панели типографики Webflow переключатель Italic не использовать. У серифа (PPPangaia) оси наклона нет — курсив там невозможен в принципе. Подробности и диагностика — `docs/typography-fonts.md`.
...
- **Текст, выравниваемый флексом, — обнулять маргины** — `align-items: center` центрует не текст, а **бокс вместе с маргинами**. Любой унаследованный `margin-top`/`margin-bottom` у надписи сдвигает её ровно на половину своего значения, и снаружи это читается как «у надписей разные шрифты», хотя кегль и интерлиньяж совпадают до десятой. Реальный кейс (22.09.2026): у подписи «Effective date:» висел `margin-top: -5px`, и базовая линия ушла вверх на 2,5px относительно соседней плашки с датой. **Диагностика:** высоты боксов совпадают, а `getBoundingClientRect().top` у них разный — значит дело в маргинах, а не в типографике; мерить базовую линию, вставив в конец текста пустой `<span>` нулевой ширины и сравнив `bottom`. **`align-items: baseline` помогает не всегда:** если сосед сам флекс-контейнер, наружу он отдаёт не базовую линию своего текста (расхождение упало с 4,15 до 3,3px и осталось). Надёжный приём — **сделать боксы одинаковой высоты**: одинаковый вертикальный паддинг с обеих сторон, обнулённые маргины, центрирование; при равных интерлиньяжах линии совпадают арифметически, а не на глаз.
