...
- **Имена слоёв и классов в Webflow — только на английском** — то же правило, что и для комментариев в коде, и по той же причине: имя слоя в Navigator, имя класса, название компонента и его описание живут в **общем воркспейсе**, их читают коллеги, и они переживают задачу, в которой появились. Кириллица там читается как временная пометка «для себя» и мешает поиску. Касается и служебных подписей, которые ставятся, чтобы передать работу человеку (напр. `VIS-1 → Status ≠ In force`) — они тоже остаются в файле после того, как работа принята. Обсуждение в чате при этом на языке пользователя, как и раньше. Правило от 23.09.2026: в юр-шаблоне слои были названы по-русски, пользователь поправил.
