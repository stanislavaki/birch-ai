...
- **Имена слоёв и классов в Webflow — только на английском** — то же правило, что и для комментариев в коде, и по той же причине: имя слоя в Navigator, имя класса, название компонента и его описание, **имя поля CMS и подсказка под ним** живут в **общем воркспейсе**, их читают коллеги, и они переживают задачу, в которой появились. Кириллица там читается как временная пометка «для себя» и мешает поиску. Подсказка поля CMS — случай с добавкой: её читает не разработчик, а редактор (юрист, контент-менеджер) в момент заполнения записи, и часто это единственная документация, которая до него доходит, — поэтому писать её не только по-английски, но и по существу, объясняя правило целиком, а не одной строкой-напоминанием. Касается и служебных подписей, которые ставятся, чтобы передать работу человеку (напр. `VIS-1 → Status ≠ In force`) — они тоже остаются в файле после того, как работа принята. Обсуждение в чате при этом на языке пользователя, как и раньше. Правило от 23.09.2026: в юр-шаблоне слои были названы по-русски, пользователь поправил.
...
- **Вложенные скругления — внутренний радиус = внешний минус паддинг** — у формы внутри скруглённой формы радиус не копируется, а вычисляется: панель радиусом 12 с паддингом 8 требует у внутренней строки радиус **4**. Если поставить обеим 12, углы идут по разной кривизне, между ними появляется неравномерный зазор, и это читается как неаккуратность, хотя на словах «оба по 12» звучит согласованно. Правило работает в обе стороны: увеличили паддинг — уменьшайте внутренний радиус. Касается всего вложенного: карточка в карточке, чип в плашке, строка в выпадающем списке, плашка метрики в кейсе. Реальный кейс (22.09.2026): строки селектора редакций стояли на 12 при панели 12 и паддинге 8 — расхождение заметил арт-дир.
