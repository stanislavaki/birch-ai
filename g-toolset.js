...
(() => {
  ...
  if (document.getElementById('gridOverlay')) return;   // idempotent
  ...
  function buildPanel() {
    ...
    // typography (only when this element directly contains text)
    ...
    copy.addEventListener('click', async () => {
      ...
      if (!ok) {
        // legacy fallback for browsers/contexts where the modern API is blocked
  ...
  // ── inspector event handlers (only attached while active) ─────────────
  function onMove(e) {
    if (!active) return;
    if (panelDrag) return;                // suppress hover-highlight while dragging panel
    if (isOurUI(e.target)) { if (!selected) highlight.style.display = 'none'; return; }
    if (selected) return;                 // freeze highlight on selected
    placeHighlight(e.target);
  }

  function onClickCapture(e) {
    if (!active) return;
    if (isOurUI(e.target)) return;        // let panel inputs receive the click
    e.preventDefault();
    e.stopPropagation();
    if (selected === e.target) {          // second click on same element → deselect
      selected = null;
      highlight.style.display = 'none';
      buildPanel();
      return;
  ...
  function onMouseDownCapture(e) {
    ...
    e.preventDefault();                   // suppress text selection on inspect-clicks
  }
  ...
  // ── G key toggles the whole toolset ───────────────────────────────────
